...
from data_load.vision import ImageList_Active
from utils import set_rngs
...
def digit_load(args): 
    ...
    if args.dset == 's2m':
        ...
        test_target = mnist.MNIST('./data/mnist/', train=False, download=True,
                    transform=transforms.Compose([transforms.Resize(32)]))                    # transforms.Lambda(lambda x: x.convert("RGB")),
    ...
    dset_loaders["target"] = DataLoader(train_target, batch_size=train_bs, shuffle=True, 
        num_workers=args.worker, drop_last=False)
    dset_loaders["pool"] = DataLoader(train_target, batch_size=train_bs, shuffle=False, 
        num_workers=args.worker, drop_last=False)
...
def train_target(args):
    ## set BNN
    bnn = ConvolutionalNeuralNetwork((1,32,32), 10, 0.5).cuda()
    
    dset_loaders, pool_dset = digit_load(args)
    ## set base network
    if args.dset == 'u2m':
        netF = network.LeNetBase().cuda()
    elif args.dset == 'm2u':
    ...
    while iter_num < max_iter:
        ...
        ## obtain active label
        active_samples, active_labels = obtain_active_label(dset_loaders["pool"], pool_dset, dset_loaders["test"], train_list, inputs_test, bnn, args)
        ...
        active_labels = torch.from_numpy(np.asarray(active_labels)).cuda()
...
def obtain_active_label(pool_dloader, pool_dset, val_dloader, train_list, target_inputs, model, args) :
    ## move supervised label from BNN to DNN
    print("Setting training dataloader")
    if len(train_list) == 0 :
        dummy_data = next(iter(pool_dloader))
        train_list.append((dummy_data[0], dummy_data[1]))
    ...
    with torch.inference_mode():
        scores = trainer.estimate_uncertainty(
            ...
            mode="epig",
            rng=set_rngs(0, False),
            epig_probs_target=None,
            epig_probs_adjustment=None,
            epig_using_matmul=False,
        )
    scores = scores.numpy()
    acquired_pool_inds = np.argmax(scores)

    data = pool_dset[acquired_pool_inds]
    samples = data[0]
    labels = data[1]
...
def obtain_psuedo_label(inputs_test, active_samples, active_labels, args) :
    ## psuedo labeling
